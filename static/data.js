...
* @date 11/13/14
*
* App logic. Sends 1 or 0 to local server app when any key on the keyboard is pressed.
*/
...
window.addEventListener('load', function() {
	...
	window.addEventListener('keydown', function(e) {
		// if key has not been registered, add it to list of pressed keys
		if(!keys.pressed[e.keyCode]) {
			keys.pressed[e.keyCode] = true;

			// output one
			sid.writeToStatsCounterOne(1);
			sid.command('/value/1');
		}
	});
